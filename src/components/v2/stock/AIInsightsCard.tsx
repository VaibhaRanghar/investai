      ...
      <Card variant="gradient">
        <CardContent className="py-12">
          <div className="flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-gray-600 font-medium">AI is analyzing the stock...</p>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardContent className="space-y-6">
        ...
        {targets && (
          <div className="bg-white rounded-lg p-4 border border-indigo-200">
            <h4 className="font-semibold text-gray-900 mb-3">Suggested Levels</h4>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-xs text-gray-500 mb-1">Entry</p>
                <p className="text-lg font-bold text-green-600">₹{targets.entry}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Stop Loss</p>
                <p className="text-lg font-bold text-red-600">₹{targets.stopLoss}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Target</p>
                <p className="text-lg font-bold text-indigo-600">₹{targets.target}</p>
              </div>
            </div>
          </div>
        )}
  ...
  insights: [
    {
      type: 'positive',
      category: 'Valuation',
      message: 'P/E at 42.8 is below sector average (43.78), indicating fair to slight undervaluation'
    },
    {
      type: 'positive',
      category: 'Quality',
      message: 'Strong promoter holding (62.4%) stable for 5 quarters shows management confidence'
    },
    {
      type: 'positive',
      category: 'Fundamentals',
      message: 'Healthy 27% profit margin and regular dividend payer (₹8/share annually)'
    },
    {
      type: 'neutral',
      category: 'Technical',
      message: 'At 24% of 52W range - good risk-reward but needs volume confirmation'
    },
    {
      type: 'negative',
      category: 'Risk',
      message: 'Recent management change (Oct 1) and weak pre-market sentiment require monitoring'
    }
  ],
  targets: {
    entry: 700,
